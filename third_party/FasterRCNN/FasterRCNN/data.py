...
from .common import (CustomResize, DataFromListOfDict, box_to_point8,
                    filter_boxes_inside_shape, np_iou, point8_to_box,
                    polygons_to_mask)
from .config import config as cfg
from .dataset import DatasetRegistry#, register_coco, register_voc
from .utils.np_box_ops import area as np_area
from .utils.np_box_ops import ioa as np_ioa

import tensorpack.utils.viz as tpviz
from tensorpack.dataflow.imgaug import ImageAugmentor
from tensorpack.dataflow.imgaug import RandomChooseAug
...
class RandomChooseMultiAug(ImageAugmentor):
  """ Randomly choose one from a list of augmentors

    Extended to multilayer in order to follow Randaugment Fashion.
    """

  def __init__(self, aug_lists, layer=2):
    """
        Args:
            aug_lists (list): list of augmentors, or list of (augmentor,
              probability) tuples
    """
    if isinstance(aug_lists[0], (tuple, list)):
      ...
      self._init(locals())
    self._layer = layer
    super(RandomChooseMultiAug, self).__init__()

  def reset_state(self):
    super(RandomChooseMultiAug, self).reset_state()
  ...
  def get_transform(self, img):
    for _ in range(self._layer):
      ...
      img = self.aug_lists[aug_idx].get_transform(img)
    return img
...
class TrainingDataPreprocessor:
  """
    ...
    explicitly pass cfg to it, in the spirit of "explicitly pass resources to
    subprocess".
    """
  ...
  def get_multilevel_rpn_anchor_input(self, im, boxes, is_crowd):
    """
        ...
        Returns:
            ...
            Each tuple contains the anchor labels and target boxes for each
            pixel in the featuremap.
    ...
    anchors_per_level = get_all_anchors_fpn(
        ...
        sizes=self.cfg.FPN.ANCHOR_SIZES,
        ratios=self.cfg.RPN.ANCHOR_RATIOS,
        max_size=self.cfg.PREPROC.MAX_SIZE,
    )
  ...
  def get_anchor_labels(self, anchors, gt_boxes, crowd_boxes):
    """
        ...
        Returns:
            ...
            anchor_boxes: Ax4. Contains the target gt_box for each anchor when
            the anchor is fg.
        """
    ...
    anchor_labels[
        ious_max_per_anchor >= self.cfg.RPN.POSITIVE_ANCHOR_THRESH] = 1
