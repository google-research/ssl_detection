...
def proposal_metrics(iou):
  ...
  mean_best_iou = tf.cond(
      tf.math.equal(tf.size(best_iou), 0),
      lambda: tf.constant(0., tf.float32),
      lambda: tf.reduce_mean(best_iou),
      name='best_iou_per_gt')
  summaries = [mean_best_iou]
  with tf.device('/cpu:0'):
    for th in [0.3, 0.5]:
      recall = tf.math.divide_no_nan(  #tf.truediv(
          tf.cast(tf.count_nonzero(best_iou >= th), tf.float32),
          tf.cast(tf.size(best_iou, out_type=tf.int64), tf.float32),
...
def sample_fast_rcnn_targets(boxes, gt_boxes, gt_labels):
  """
    ...
    #fg is guaranteed to be > 0, because ground truth boxes will be added as
    proposals.
    ...
    Returns:
        A BoxProposals instance, with:
            ...
            sampled_labels: t int64 labels, in [0, #class). Positive means
            foreground.
...
def fastrcnn_losses(labels, label_logits, fg_boxes, fg_box_logits):
  ...
  box_loss = tf.reduce_sum(
      tf.abs(fg_boxes - fg_box_logits), name='box_loss_unormalized')

  add_moving_summary(box_loss)
  add_moving_summary(
      tf.identity(
          tf.cast(tf.shape(labels)[0] == 0, tf.float32),
          name='detect_empty_labels'))

  # TODO(zizhaoz): check whether divide_no_nan sovles the nan issue
  box_loss = tf.math.divide_no_nan(
      box_loss, tf.cast(tf.shape(labels)[0], tf.float32), name='box_loss')
  add_moving_summary(tf.identity(box_loss, name='box_loss_debug'))
...
def fastrcnn_predictions(boxes, scores):
  ...
  selection = tf.image.non_max_suppression(nms_boxes, filtered_scores,
  ...
  def __init__(self, boxes, labels=None, fg_inds_wrt_gt=None):
    """
        Args:
            ...
            fg_inds_wrt_gt: #fg, each in [0, M)  The last four arguments could
              be None when not training.
    """
...
class FastRCNNHead(object):
  """
    A class to process & decode inputs/outputs of a fastrcnn
    classification+regression head.
    """

  def __init__(self, proposals, box_logits, label_logits, gt_boxes,
  ...
  def decoded_output_boxes_for_label(self, labels):
    ...
    decoded = decode_bbox_target(needed_logits / self.bbox_regression_weights,
