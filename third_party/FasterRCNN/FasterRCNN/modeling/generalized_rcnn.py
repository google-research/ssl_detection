...
from .model_frcnn import (BoxProposals, FastRCNNHead, fastrcnn_outputs,
                          fastrcnn_predictions, sample_fast_rcnn_targets)
...
def visualize_images(image, boxes, name):
  """
    images: BxHxWx3
  ...
  with tf.name_scope('imgbox'):
    ...
    boxes = tf.stack(
        [boxes[:, 1] / h, boxes[:, 0] / w, boxes[:, 3] / h, boxes[:, 2] / w], 1)
...
class GeneralizedRCNN(ModelDesc):
  ...
  def build_graph(self, *inputs):
    ...
    visualize_images(image, inputs['gt_boxes'], name='labeled')
...
class ResNetC4Model(GeneralizedRCNN):
  ...
  def roi_heads(self, image, features, proposals, targets):
    ...
    if self.training:
      ...
      if cfg.MODE_MASK:
        roi_resized = roi_align(featuremap,
                                final_boxes * (1.0 / cfg.RPN.ANCHOR_STRIDE), 14)
  ...
  def roi_heads(self, image, features, proposals, targets):
    ...
    if self.training:
      ...
      if cfg.MODE_MASK:
        ...
        roi_feature_maskrcnn = multilevel_roi_align(features[:4], final_boxes,
                                                    14)
        maskrcnn_head_func = getattr(model_mrcnn, cfg.FPN.MRCNN_HEAD_FUNC)
        mask_logits = maskrcnn_head_func(
            'maskrcnn', roi_feature_maskrcnn,
