...
def collect_samples(model,
  ...
  with tqdm.tqdm(total=nr_visualize) as pbar:
    for idx, dp in itertools.islice(enumerate(df), nr_visualize):
      ...
      viz = tpviz.stack_patches([gt_viz, bg_viz, fg_viz, final_viz], 2, 2)
...
def do_visualize(model, model_path, nr_visualize=100, output_dir='output'):
  ...
  with tqdm.tqdm(total=nr_visualize) as pbar:
    for idx, dp in itertools.islice(enumerate(df), nr_visualize):
      ...
      viz = tpviz.stack_patches([gt_viz, proposal_viz, score_viz, final_viz], 2,
                                2)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--sample', action='store_true', help='visualize intermediate results')
  parser.add_argument(
      '--evaluate',
      help='Run evaluation. '
  ...
  try:
    ...
    logger.warning('COCO does not find!')
  ...
  #output_dir = os.path.join('output', args.load.split('train_log')[1].lstrip('/'))
  output_dir = os.path.join('output', args.load.split('VOC2007')[1].lstrip('/'))
  if args.sample:
    collect_samples(MODEL, args.load, output_dir=output_dir)
