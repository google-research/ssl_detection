...
class CustomResize(ImageAugmentor):
  ...
  def __init__(self, short_edge_length, max_size, interp=cv2.INTER_LINEAR):
    """
        Args:
            short_edge_length ([int, int]): a [min, max] interval from which to
              sample the shortest edge length.
...
class CustomResize2(ImageAugmentor):
  """
    Try resizing the shortest edge to a certain number
    while avoiding the longest edge to exceed max_size.

    v2 version keep the last random sampled size.
    It is used mainly for generating paired weak and strong augmentation.
    """
  ...
  def __init__(self, short_edge_length, max_size, interp=cv2.INTER_LINEAR):
    """
        Args:
            ...
            max_size (int): maximum allowed longest edge length.
    """
    super(CustomResize2, self).__init__()
    if isinstance(short_edge_length, int):
      short_edge_length = (short_edge_length, short_edge_length)
    self._init(locals())
    self.size = None
...
def polygons_to_mask(polys, height, width):
  """
    ...
    Args:
        polys: a list of nx2 float array. Each array contains many (x, y)
          coordinates.
