...
# do not append these arguments in log_dir pathname
IGNORE_ARGS = [
    'DATA.TRAIN', 'BACKBONE.WEIGHTS', 'DATA.BASEDIR', 'TRAIN.STARTING_EPOCH',
    'TRAIN.EVAL_PERIOD', 'DATA.NUM_WORKERS'
]
...
class AttrDict():
  ...
  def update_args(self, args):
    ...
    for cfg in args:
      ...
      if keys not in IGNORE_ARGS:
        suffix.append('{}{}'.format(keys, v))
...
_C.DATA.VAL = ('coco_val2017',)  # AKA mcinival2014
...
_C.FRCNN.BBOX_REG_WEIGHTS = [10., 10., 5.,
                             5.]  # Slightly better setting: 20, 20, 10, 10
...
_C.FPN.ANCHOR_SIZES = (32, 64, 128, 256, 512)  # fix
...
def finalize_configs(is_training):
  ...
  if isinstance(
      _C.DATA.VAL,
      six.string_types):  # support single string (the typical case) as well
  ...
  assert len(_C.FPN.ANCHOR_STRIDES) == len(_C.FPN.ANCHOR_SIZES)
  ...
  if is_training:
    ...
    assert ngpu % 8 == 0 or 8 % ngpu == 0, (
        'Can only train with 1,2,4 or >=8 '
