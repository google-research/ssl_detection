...
class DatasetSplit():
  """
    A class to load datasets, evaluate results for a datast split (e.g.,
    "coco_train_2017")
  ...
  def training_roidbs(self):
    """
        ...
        Produce "roidbs" as a list of dict, each dict corresponds to one image
        with k>=0 instances.
        and the following keys are expected for training:

        file_name: str, full path to the image
        boxes: numpy array of kx4 floats, each row is [x1, y1, x2, y2]
        class: numpy array of k integers, in the range of [1, #categories], NOT
        [0, #categories)
        ...
        segmentation: k lists of numpy arrays.
            ...
            If your segmentation annotations are originally masks rather than
            polygons,
            either convert it, or the augmentation will need to be changed or
            skipped accordingly.
  ...
  def inference_roidbs(self):
    """
        Returns:
            ...
            file_name (str): full path to the image
            image_id (str): an id for the image. The inference results will be
            stored with this id.
        """
    raise NotImplementedError()
  ...
  def eval_inference_results(self, results, output=None):
    """
        Args:
            results (list[dict]): the inference results as dicts. Each dict
              corresponds to one __instance__. It contains the
                  following keys:
                image_id (str): the id that matches `inference_roidbs`.
                category_id (int): the category prediction, in range [1,
                  #category]
                bbox (list[float]): x1, y1, x2, y2
                score (float):
                segmentation: the segmentation mask in COCO's rle format.
            output (str): the output file or directory to optionally save the
              results to.
...
class DatasetRegistry():
  ...
  def register(name, func):
    ...
    """
    assert name not in DatasetRegistry._registry, ('Dataset {} was registered '
                                                   'already!').format(name)
